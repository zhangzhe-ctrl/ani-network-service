...
- 退出恢复须区分正常取消后的确定结果落库、真正未知写入和失效 lease；有界完成事务遵循 [VPC 生命周期规则](../specs/vpc-subnet.md)。历史缺失回执的处理另见[受控恢复提案](provider-unknown-create-recovery.md)，不能由一次 NotFound 或当前修正通过推断旧请求未发送。
...
2026-09-17 增量：用户明确显式健康检查端口。实现遵循[端口契约](../specs/vpc-connectivity-lb.md#61-资源和租户输入)，覆盖创建/更新校验、配置版本存储、查询及 Provider 映射；本次仅针对性检查和编译，不重跑完整集群验收。
