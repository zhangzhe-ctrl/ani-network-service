...
普通 Egress/LB 观察发现缓存审计失效，或 LB 关系覆盖不足时，可在原调用上下文及超时预算内重新完整采集一次，再执行相同的时效、身份、覆盖和并发围栏检查。新采集仍不满足条件时返回原有暂时不可用结果，由持久调度恢复；不得无限重试、延长调用预算、接受替换后的 UID，或仅刷新缓存时间戳来维持就绪状态。
