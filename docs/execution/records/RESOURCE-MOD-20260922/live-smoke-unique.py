# Corrected matrix for a fresh acceptance run; not executed in rsmod-0922.
# Ordinary Pod communication is already covered by isolation-smoke positive controls.
# Existing per-version started markers continue to prohibit rerunning this run.
...
for source in instances:
 cases=[('private-lb','http://'+p['driver']['private_vip']+':8081/',{x['instance_id'] for x in instances})]
