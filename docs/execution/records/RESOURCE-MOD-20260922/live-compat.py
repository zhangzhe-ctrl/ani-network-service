...
os.umask(0o077)
r=pathlib.Path('/home/chabking/workspace/ani-resource-service-runs/rsmod-20260921T1640Z');p=r/'private/runtime';plan=json.loads((p/'plan.json').read_text());s=json.loads((p/'product-driver/state.json').read_text())
...
def call(method,body,socket='tenant-a.sock'):
 cmd=[str(r/'baseline/bin/lb-api'),'call','-target','unix://'+str(pathlib.Path(plan['sockets'])/socket),'-service','NetworkService','-method',method]
...
if action=='freeze':
 ...
 for f in sorted(d.glob('create-*.json')):
  ...
  assert current['state']=='RESOURCE_STATE_AVAILABLE' and not current['observation_stale'] and not current['reason'],current
 ...
 body={'tenant_id':plan['tenant_id'],'vpc_id':s['resources']['vpc']['id'],'name':'rsmod-0922-candidate-'+label,'cidr':'10.236.'+('2' if label=='normal' else '3')+'.0/24','idempotency_key':'rsmod-0922-candidate-'+label}
