...
`7b39e53` 上的 `make verify`、`make audit`、构建及镜像入口检查均退出 0，见
[交付核对](evidence/delivery-assessment.json)与[完整审计](evidence/delivery-audit-recovery-all.txt)。
全历史 29 个提交、148.82 MB 扫描无未处理命中；漏洞零命中；SBOM 保持 60 项非主运行模块，
恰为上述五项版本变化。原 notice 校验不变，各非主模块均具有许可证证据。
生成器对主模块缺少许可证证据的既有警告原样留在日志，未将其隐去或扩大既有门禁范围。

与 PG/race 受测快照比较 273 个运行/工具文件，仅 `.gitleaks.toml` 为上述审计配置差异；
Go 源码、模块版本、SQL、migration、配置均逐项一致。
[镜像核验](evidence/image-check.json)固定新镜像和实际复制的二进制，使用原 digest 基础镜像；
只读、无网络、1 CPU/512 MiB、非 root 的 `-h` 对照除可执行文件名外与原版相同。
这不表示镜像已部署或新 R4 已通过。

最终记录与状态页入树后还须重生成 SBOM；最终精确提交的审计/可重生成回执、分支推送和
CI 关联放在 Fedora 本子目录 evidence 中及 PR #4，避免把含自身 SHA 的回执写回造成递归提交。
当前缺少独立 Public 资源事实，新的完整 R4 尚未启动，合入、仓库/正式目录改名和改名后
模块消费仍为 not_verified。已授权的下一次 R4 必须先取得设备/平台入口、地址范围、网关及归属，
重核当前集群身份和保护清单，再使用新 run 与修正后的唯一采样矩阵执行；不得重用旧流量标记或恢复旧备份冒充接管。
