...
[[ -L .tools && $(readlink .tools) == "$TASK_RUN/publication/.tools" ]]
unlink .tools
cp -a --reflink=auto "$TASK_RUN/publication/.tools" .tools
python3 - <<'PY'
import pathlib,json,subprocess,hashlib,datetime
p=pathlib.Path('.');r=p.parent/'evidence'
...
p=pathlib.Path('.');r=p.parent/'evidence';m=json.loads((r/'source-manifest.json').read_text());bad=[x['path'] for x in m['files'] if hashlib.sha256((p/x['path']).read_bytes()).hexdigest()!=x['sha256']];assert not bad,bad
