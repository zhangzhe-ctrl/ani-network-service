...
while True:
 ...
 for f in sorted((p/'compatibility').glob('create-*.json')):
  v=json.loads(f.read_text())['accepted']['subnet'];plan=json.loads((p/'plan.json').read_text())
  a=[str(r/'bin/baseline-lb-api'),'call','-target','unix://'+plan['sockets']+'/tenant-a.sock','-service','NetworkService','-method','GetSubnet']
  reply=subprocess.run(a,input=json.dumps({'tenant_id':plan['tenant_id'],'subnet_id':v['id']}),capture_output=True,text=True,timeout=30)
  current=json.loads(reply.stdout).get('response',{}).get('subnet',{})
  deleted=(p/'compatibility'/('delete-'+f.stem.removeprefix('create-')+'.json')).exists()
  checks['extra-'+f.stem]=reply.returncode==0 and (current.get('state')=='RESOURCE_STATE_DELETED' if deleted else current.get('state')=='RESOURCE_STATE_AVAILABLE' and not current.get('observation_stale',True) and not current.get('reason'))
