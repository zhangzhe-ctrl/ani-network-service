...
for d in support['deletions']:
 try:urllib.request.urlopen('http://127.0.0.1:44959'+d['uri'],timeout=30);raise RuntimeError('support object retained '+d['uri'])
...
unit=subprocess.run(['systemctl','--user','is-active','--quiet','rsctl-0922-serve']);assert unit.returncode!=0
...
pg=json.loads((p/'postgres.json').read_text())['container'];container=json.loads(subprocess.check_output(['docker','inspect',pg]))[0];assert container['Name']=='/rsctl-0922-pg'
