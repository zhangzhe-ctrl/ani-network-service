...
r=pathlib.Path('/home/chabking/workspace/ani-resource-service-runs/rsmod-20260921T1640Z/control-20260922T0400Z');p=r/'private/runtime';pg=json.loads((p/'postgres.json').read_text())['container'];queries={}
...
for name,q in queries.items():
 x=subprocess.run(['docker','exec',pg,'psql','-U','postgres','-d','net_vpc_lb_02_c92201','-At','-c',q],capture_output=True,text=True);assert x.returncode==0,x.stderr;results[name]={'sql':q,'exit':x.returncode,'result':json.loads(x.stdout.strip() or 'null')}
...
record={'at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'host':'fedora','database':'net_vpc_lb_02_c92201','result':'pass','queries':results}
