...
load=subprocess.check_output(['systemctl','--user','show',unit,'-p','LoadState','--value'],text=True).strip()
if load=='loaded':subprocess.run(['systemctl','--user','stop',unit],check=True,timeout=60)
else:assert load=='not-found',load
