...
try:
 ...
 raw=subprocess.check_output([str(r/'baseline/bin/lb-api'),'call','-target','unix://'+plan['sockets']+'/tenant-a.sock','-service','NetworkService','-method','GetOperation'],input=json.dumps(request).encode(),timeout=25)
 ...
 subprocess.run(['systemctl','--user','stop','resource-live-20260922'],check=True,timeout=60)
