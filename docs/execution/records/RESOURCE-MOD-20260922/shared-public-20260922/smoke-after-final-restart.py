...
assert not (r/'traffic-after-final-restart.jsonl').exists()
...
(r/'traffic-after-final-restart-matrix.json').write_text(json.dumps({'scope':'single bounded post-replacement environment check; not R4 old/candidate comparison','cases':cases},indent=2)+'\n')
results=[]
for c in cases:
 for attempt in range(1,7):
  nonce=f'shared-public-final-20260922-{c["case"]}-{c["source"]}-{attempt}'
  ...
  with (r/'traffic-after-final-restart.jsonl').open('a') as f:f.write(json.dumps(row)+'\n')
...
(r/'traffic-after-final-restart-summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary),flush=True)
