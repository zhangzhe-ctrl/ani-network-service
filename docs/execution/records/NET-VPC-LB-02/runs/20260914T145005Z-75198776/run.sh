...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260914T145005Z-75198776
printf '%s  source.tar.gz\n' de05950e95f33f05f72de06c81b9a3d759273a7ecf9c6b86c49ce34a4218d006 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard bash -c '.tools/bin/sqlc generate && scripts/integration -run "TestBaseSchemaEmptyMigrationAndRuntimeRole|TestBaseSchemaTypedTenantPlacementAndVIPConstraints|TestBaseSchemaLegacyPublicUpgradePreservesDurableFactsAndReplay" ./internal/data' 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
