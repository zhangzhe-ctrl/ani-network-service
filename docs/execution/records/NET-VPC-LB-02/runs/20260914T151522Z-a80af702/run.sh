...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260914T151522Z-a80af702
printf '%s  source.tar.gz\n' 12531abf580eceb4a33ef04a48ff99ac14a5c98e893ca0487861449d2b498dc1 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard bash -c 'make generate && go test -count=1 ./internal/biz ./internal/service ./internal/conf/v1 ./cmd/ani-network-service && scripts/integration -run "TestLBAdmission|TestBaseSchema" ./internal/data' 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
