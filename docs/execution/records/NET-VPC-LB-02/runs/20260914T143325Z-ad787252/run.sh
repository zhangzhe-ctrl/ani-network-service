...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260914T143325Z-ad787252
printf '%s  source.tar.gz\n' 2a65bef8fab4b7f79d0db35ec7cef9d5b235ea9997d1ca8f128714ac3cf7cf37 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard go test -count=1 ./internal/biz ./internal/service 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
