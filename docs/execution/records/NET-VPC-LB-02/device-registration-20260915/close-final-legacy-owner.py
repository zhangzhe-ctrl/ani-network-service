"""Close only this run's two final-legacy-test submissions; retain historical Pod UIDs."""
...
p=pathlib.Path('/home/ubuntu/workspace/ani-network-service-runs/lb02-09141908-2b3122/private');registry=p/'owner.json';plan=p/'final-legacy-owner-close-plan.json';entries=json.loads(registry.read_text());wanted={'att_e17cd1ca47b741e6a637c44fbc7789cb': 'lb02-2b3122-legacy-a', 'att_c3ecbb00f69442868d10004d061a5ffe': 'lb02-2b3122-legacy-b', 'att_27718b5a92e64f669739aec7a5c51f8a': 'lb02-2b3122-legacy-client'};selected=[x for x in entries if x['attachment_id'] in wanted];assert len(selected)==3
...
def call(args,body=None):
 ...
 with (p/'final-legacy-close-actions.jsonl').open('a') as out:out.write(json.dumps(v)+'\n')
 assert r.returncode==0,v;return r.stdout
if phase=='begin':
 assert not plan.exists();saved=registry.read_bytes();backup=p/'owner.before-final-legacy-close.json';assert not backup.exists();backup.write_bytes(saved);backup.chmod(0o600)
 for e in selected:
 ...
 pending=p/'owner.final-legacy-closing.json';pending.write_text(json.dumps(entries));pending.chmod(0o600);pending.replace(registry)
 for e in selected:
 ...
 pending=p/'owner.final-legacy-closed.json';pending.write_text(json.dumps(entries));pending.chmod(0o600);pending.replace(registry)
