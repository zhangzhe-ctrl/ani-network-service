"""Close only this run's two final-base-test submissions; retain historical Pod UIDs."""
...
p=pathlib.Path('/home/ubuntu/workspace/ani-network-service-runs/lb02-09141908-2b3122/private');registry=p/'owner.json';plan=p/'final-base-owner-close-plan.json';entries=json.loads(registry.read_text());wanted={'att_7d08b61f4038432a842d2fda525607b9': 'lb02-2b3122-base-a', 'att_a6172d5ee5be434087f939001a435c9e': 'lb02-2b3122-base-b', 'att_dc37931e4bfa4d3d9fe7a2ed3b791e62': 'lb02-2b3122-base-client'};selected=[x for x in entries if x['attachment_id'] in wanted];assert len(selected)==3
...
def call(args,body=None):
 ...
 with (p/'final-base-close-actions.jsonl').open('a') as out:out.write(json.dumps(v)+'\n')
 assert r.returncode==0,v;return r.stdout
if phase=='begin':
 assert not plan.exists();saved=registry.read_bytes();backup=p/'owner.before-final-base-close.json';assert not backup.exists();backup.write_bytes(saved);backup.chmod(0o600)
 for e in selected:
 ...
 pending=p/'owner.final-base-closing.json';pending.write_text(json.dumps(entries));pending.chmod(0o600);pending.replace(registry)
 for e in selected:
 ...
 pending=p/'owner.final-base-closed.json';pending.write_text(json.dumps(entries));pending.chmod(0o600);pending.replace(registry)
