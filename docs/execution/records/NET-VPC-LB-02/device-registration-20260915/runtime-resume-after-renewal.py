"""Resume the preserved U09 run after the collector-renewal candidate passes its gates."""
import datetime
import hashlib
import json
import os
...
source_run = pathlib.Path('/home/ubuntu/workspace/ani-network-service-runs/snat-20260915T034538Z-b890832c')
...
gates = json.loads((source_run/'renewal-candidate-gates.json').read_text())
assert gates['result'] == 'pass'
snapshot = json.loads((source_run/'snapshot.json').read_text())
assert snapshot['archive_sha256'] == 'fc01f175058b55eb89399d615fb2dda6e7927460369690217e490515dbb6d5c4'
...
backup = private/'before-renewal-fix-r10'
...
for name in before:
    ...
    replacement = private/'bin'/(name+'.candidate-r10')
...
plan['source_build_run'] = '20260915T034538Z-b890832c'
plan.pop('temporary_diagnostic_build_run', None)
pending = private/'plan.candidate-r10.json'
...
unit = 'ani-net-lb02-2b3122-r10.service'
