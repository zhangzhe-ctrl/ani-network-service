#!/usr/bin/env bash
# Retry after temporary executable disappearance; isolate TMPDIR within this run.
set -Eeuo pipefail
...
test ! -e ../audit-final-retry-02.exit
test ! -e ../audit-final-retry-02.log
test ! -e ../artifacts
mkdir -m 700 ../audit-final-tmp-02
export TMPDIR="$(realpath ../audit-final-tmp-02)"
python3 - <<'CHECK'
...
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock \
  ...
  scripts/net05a-resource-guard bash -e -c '
    scripts/integration -race ./internal/data -timeout 12m -json
    mkdir ../artifacts
  ...
  ' 2>&1 | tee ../audit-final-retry-02.log
result=$?
set -e
printf '%s\n' "$result" > ../audit-final-retry-02.exit
exit "$result"
