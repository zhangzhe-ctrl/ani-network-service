...
for target in ["172.16.102.2"] + ["172.16.102."+str(i) for i in range(192,208)]:
    results.append(attempt(target,"0.0.0.0",None))
print(json.dumps({"at":datetime.datetime.now(datetime.timezone.utc).isoformat(),"interface":interface,"mac":mac.hex(":"),"cases":results,"host_configuration_changes":0,"interpretation":"No candidate reply is conflict-screening only, not proof of reservation. Gateway ordinary-source ARP is a link diagnostic, not public egress verification."}))
