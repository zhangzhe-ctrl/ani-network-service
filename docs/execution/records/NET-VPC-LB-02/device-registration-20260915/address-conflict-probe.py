...
results = [attempt("172.16.102.1", "172.16.102.200", None)]
for suffix in [2, *range(192, 208)]:
    target = "172.16.102." + str(suffix)
    results.append(attempt(target, "0.0.0.0" if suffix == 200 else "172.16.102.200", None))
results.append(attempt("172.16.102.1", "172.16.102.200", None))
print(json.dumps({"at": datetime.datetime.now(datetime.timezone.utc).isoformat(), "interface":interface, "mac":mac.hex(":"), "cases":results, "host_configuration_changes":0, "source_address_assigned":False, "interpretation":"Bounded ARP conflict check, not an IPAM/DHCP reservation. Gateway replies are the positive control; no reply alone is inconclusive."}))
