"""Resume the preserved U09 run after the canonical observation hash candidate passes its gates."""
import datetime
import hashlib
import json
import os
...
source_run = pathlib.Path('/home/ubuntu/workspace/ani-network-service-runs/snat-20260915T051609Z-70cbcaa3')
...
gates = json.loads((source_run/'canonical-candidate-gates.json').read_text())
assert gates['result'] == 'pass'
snapshot = json.loads((source_run/'snapshot.json').read_text())
assert snapshot['archive_sha256'] == '51864a0e90d621a61c9be408ad8d0216068b75b1050b0dc09395ee01f506a4a0'
...
backup = private/'before-canonical-fix-r13'
...
for name in before:
    ...
    replacement = private/'bin'/(name+'.candidate-r13')
...
plan['source_build_run'] = '20260915T051609Z-70cbcaa3'
plan.pop('temporary_diagnostic_build_run', None)
pending = private/'plan.candidate-r13.json'
...
unit = 'ani-net-lb02-2b3122-r13.service'
