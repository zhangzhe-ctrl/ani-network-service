"""Resume the preserved U09 run once the same candidate's remaining gates pass."""
import datetime
import hashlib
import json
import os
...
source_run = pathlib.Path('/home/ubuntu/workspace/ani-network-service-runs/snat-20260915T025202Z-d88ee066')
...
gates = json.loads((source_run/'audit-candidate-gates.json').read_text())
assert gates['result'] == 'pass'
snapshot = json.loads((source_run/'snapshot.json').read_text())
assert snapshot['archive_sha256'] == '17d30aa89b7f9d100120166c4c6c218038c01f7fd8b8627a7c874a658980dbad'
...
backup = private/'before-audit-fix-r9'
...
for name in before:
    ...
    replacement = private/'bin'/(name+'.candidate-r9')
...
plan['source_build_run'] = '20260915T025202Z-d88ee066'
plan.pop('temporary_diagnostic_build_run', None)
pending = private/'plan.candidate-r9.json'
...
unit = 'ani-net-lb02-2b3122-r9.service'
