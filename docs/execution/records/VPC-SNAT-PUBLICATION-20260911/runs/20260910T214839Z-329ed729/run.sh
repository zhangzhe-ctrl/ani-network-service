...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T214839Z-329ed729
printf '%s  source.tar.gz\n' c1505deba44da758ba7758de3b279a315de9f8d9ce87fb5bc5088580179a8194 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard bash -c 'make verify && make tenant-mutations && make audit' 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
