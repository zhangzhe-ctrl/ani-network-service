...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260914T130243Z-bc4f4518
printf '%s  source.tar.gz\n' 1f0436b89baa9e7c099283ffb390883ca035ac0917e490792b9fc267a65f5230 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard bash -c '.tools/bin/sqlc generate' 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
