...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260914T131819Z-a262e0a0
printf '%s  source.tar.gz\n' 7dd325545d14e02fc5165cc411e09d7ab40f82e61107c7068c0f8e4af3a6ad8b | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard bash -c '.tools/bin/sqlc generate && go test ./internal/biz ./internal/service ./internal/data ./cmd/ani-network-service -run "^$" && scripts/integration ./internal/data -run "Test(BaseSchema|BaseConnectivity|Intranet|PublicTenant|PublicBind|PublicEIP)" -timeout 120s' 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
