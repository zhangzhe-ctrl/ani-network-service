...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T141326Z-24ea957e
printf '%s  source.tar.gz\n' f8d6e2b516ade680626db7311a25c5d8000f75b5c9513cc1c78fa544d724a66e | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard scripts/integration -p 2 -run TestEgress ./internal/data 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
