...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T134335Z-e81d973f
printf '%s  source.tar.gz\n' d4dd9e2fb56698a0866c2cf59c943f2e5e37b76384e9c3f32b1f18cb927ccbb5 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard scripts/integration -p 2 -run 'TestVPCAcceptanceIsTenantScopedAndReplaysExactly|TestPublicPoolTopology|TestEgressAuthorization|TestNodeInterfaceFacts|TestEgressFingerprints' ./internal/biz ./internal/data 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
