...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T134828Z-f915232d
printf '%s  source.tar.gz\n' 1861727b33fec962d637f3a5c26490df6c94c9145f8ca4df293b699733759981 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard go test -p 2 -count=1 ./internal/biz ./internal/data 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
