...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T144842Z-acf48246
printf '%s  source.tar.gz\n' 5d236b2a423b1214a49b1ce9f3e2feae72e46c89722eab4660a1af0620cb883d | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard scripts/integration -p 2 -race ./... 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
