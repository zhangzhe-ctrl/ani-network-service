...
cd /home/ubuntu/workspace/ani-network-service-runs/snat-20260910T143025Z-c3699866
printf '%s  source.tar.gz\n' fea28cdc148c7985b5f825c9f693aaeefe8f650a1370af3e9ee9c7cbdae036d2 | sha256sum -c -
mkdir source
...
printf '%s\n' "$$" > ../shell.pid
set +e
flock -n /home/ubuntu/.local/share/ani-network-service/net05a-heavy.lock systemd-run --user --scope --quiet -p CPUQuota=200% -p MemoryMax=2300M -p MemorySwapMax=0 env GOMAXPROCS=2 GOFLAGS=-p=2 GOMEMLIMIT=1500MiB scripts/net05a-resource-guard scripts/integration -p 2 ./... 2>&1 | tee ../command.log
result=$?
set -e
printf '%s\n' "$result" > ../command.exit
