...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_request(self):
        ...
        if self.command != 'GET' and namespace not in state['namespaces']:
            ...
            return
        record = {'id': request_id, 'identity': identity, 'method': self.command, 'path': parsed.path, 'name': name, 'dry_run': 'dryRun=' in parsed.query}
        rules = []
        ...
        connection = http.client.HTTPSConnection(upstream.hostname, upstream.port, context=upstream_tls, timeout=30)
