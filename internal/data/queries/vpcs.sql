...
VALUES (sqlc.arg(tenant_id),NULLIF(sqlc.arg(vpc_id)::text,''),NULLIF(sqlc.arg(subnet_id)::text,''),NULLIF(sqlc.arg(eip_id)::text,''),NULLIF(sqlc.arg(snat_id)::text,''),sqlc.arg(binding_id),sqlc.arg(cluster_id),sqlc.arg(namespace),sqlc.arg(provider_name),false,CASE WHEN sqlc.arg(eip_id)::text<>'' THEN 'eip' WHEN sqlc.arg(snat_id)::text<>'' THEN 'snat' WHEN sqlc.arg(subnet_id)::text<>'' THEN 'subnet' ELSE 'vpc' END);
