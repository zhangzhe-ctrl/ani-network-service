...
**Intranet Address Pool（基础内网地址池）**:
由平台拥有、为租户 VPC 的基础内网出口分配地址的范围；与租户工作负载 Subnet 和公网地址池分别管理。
_Avoid_: 租户私有子网、可由租户任选的内部网段

**EIP（弹性地址）**:
由平台地址池分配、具有稳定身份及明确使用方的地址资源；分为基础内网地址与公网地址，是否可被互联网路由取决于平台出口网络。租户的“申请 EIP”功能专指公网地址。
_Avoid_: 工作负载私网地址、节点管理地址、已开启出网的同义词

**VPC Base Connectivity（VPC 基础内网连接）**:
随租户 VPC 建立并由系统维护的内网出口能力，其生命周期从属于 VPC，独立于租户是否开启公网出站。
_Avoid_: VPC 内部子网、租户可单独解绑的公网出口
...
**EIP Binding（弹性地址绑定）**:
将一个弹性地址独占关联到一个使用目标的关系；目标可以是 VPC 出站转换绑定，也可以是负载均衡实例。
_Avoid_: 任意数量的并行地址使用、只存在界面上的选择

**LoadBalancer（负载均衡实例）**:
属于租户并依附于一个 VPC、通过监听入口将请求分发到后端的网络资源；可提供私网、公网或同时提供两种入口。
_Avoid_: VPC 公网出站、子网网关、ANI Gateway

**Listener（监听器）**:
负载均衡实例对外接收请求的协议和端口定义，关联请求转发规则与后端。
_Avoid_: 后端服务端口、负载均衡实例本身

**Backend Member（后端成员）**:
负载均衡实例可以向其转发流量的业务地址及端口，带有可校验的租户和 VPC 归属。
_Avoid_: 由 Network 创建的业务实例、任意未归属地址
